...
| Unit (refresh-keys, so-startup, llama-cpp inventory, spec-decode, autotune, agent-use, clean-orphans, module-versions) | `tests/unit/*.bats` | 65 | 120s |
...
│   ├── unit/                          # BATS unit tests (65 tests: 7+2+8+5+5+6+17+4+4+7)
...
- **Unit suites (65 tests overall):** CI runs `tests/unit/01`, `02`, `09`, `10`, `11`; nightly adds `05`–`08`. `04-llama-cpp-inventory` is excluded from both — it performs live downloads and mutates the host.
