...
| Integration (maintenance, model-lifecycle, backup, watchdog, refresh-keys, bench) | `tests/integration/*.bats` | 115 | 300s |
...
│   └── integration/                   # BATS integration tests (115 tests: 14+42+10+18+5+26)
└── systemd/
...
- **Integration suites (115 tests overall):** both run `tests/integration/01`–`05` plus `e2e-bench-autotune` (the e2e suite re-runs its own regression subset, so it is the slow part of the gate).
