...
The registry lives at `~/.llm/models.conf` (`$LLM_REGISTRY`) and uses a 19-field pipe-delimited
...
- Candidate pruning: skips risky combos based on free VRAM/model size and quant rating from `quant-guide.conf`.
...
| `LLM_AUTOTUNE_MAX_CTX_DISCOURAGED` | `4096` | Context cap for discouraged quants |
| `LLM_AUTOTUNE_MAX_CTX_ACCEPTABLE` | `8192` | Context cap for acceptable quants |
| `LLM_AUTOTUNE_CTX_STEP` | `2048` | Step for nearby context sweep |
| `LLM_AUTOTUNE_MAX_UBATCH_DISCOURAGED` | `512` | Ubatch cap for discouraged quants |
