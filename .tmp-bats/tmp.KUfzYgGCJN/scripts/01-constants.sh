...
function __resolve_vscode_bin() {
    ...
    # Skip expensive pwsh.exe call in CI/test environments (TAC_SKIP_PWSH=1)
    local win_user=""
    if [[ "${TAC_SKIP_PWSH:-0}" != "1" ]]
    then
...
VRAM_TOTAL_BYTES=$((4 * 1024 * 1024 * 1024))  # 4 GB RTX 3050 Ti
fi
if [[ -z "${VRAM_USABLE_PCT+x}" ]]; then
VRAM_USABLE_PCT=95       # Percentage usable after driver overhead
fi
if [[ -z "${VRAM_THRESHOLD_PCT+x}" ]]; then
VRAM_THRESHOLD_PCT=85    # Threshold for "fits in VRAM" decisions
fi
if [[ -z "${COOLDOWN_DAILY+x}" ]]; then
COOLDOWN_DAILY=86400     # 24 hours in seconds
fi
if [[ -z "${COOLDOWN_WEEKLY+x}" ]]; then
COOLDOWN_WEEKLY=604800   # 7 days in seconds
fi
if [[ -z "${LOG_MAX_BYTES+x}" ]]; then
LOG_MAX_BYTES=1048576    # 1 MB - logtrim threshold
fi
if [[ -z "${MOE_DEFAULT_CTX+x}" ]]; then
MOE_DEFAULT_CTX=8192     # Default context size for MoE models
fi
if [[ -z "${LLAMA_DRIVE_FALLBACK_BYTES+x}" ]]; then
LLAMA_DRIVE_FALLBACK_BYTES=$((200 * 1024 * 1024 * 1024))  # 200 GB
fi
