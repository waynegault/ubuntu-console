...
function tactical_help() {
    ...
    __hRow "oc health" "Gateway health probe (--json|--plain)"
    __hRow "oc tail" "Live-tail gateway journal logs (Ctrl-C to stop)"
    __hRow "oc v" "Print installed OpenClaw CLI version"
    __hRow "oc update" "Update OpenClaw CLI to latest release"
    __hRow "oc tui" "Launch OpenClaw interactive terminal UI"

    __hSection "OPENCLAW — AGENTS & SESSIONS"
    __hRow "os" "List all active sessions"
    __hRow "oa" "Show registered agents"
    __hRow "oc start" "Dispatch an agent turn (--message '<msg>')"
    __hRow "oc stop" "Delete an agent by ID (--agent <id>)"
    __hRow "oc mem-index" "Rebuild OpenClaw vector memory search index"
    __hRow "oc memory-search" "Semantic search across memory store"

    __hSection "OPENCLAW — CONFIG & LOGS"
    __hRow "oc conf" "Open openclaw.json in VS Code"
    __hRow "oc config" "Read/write OpenClaw config keys (get|set|unset)"
    __hRow "oc env" "Display all OpenClaw and LLM environment variables"
    __hRow "oc keys" "List Windows API keys bridged to WSL"
    __hRow "oc ms" "Probe all configured model provider endpoints"
    __hRow "oc doc-fix" "Run openclaw doctor --fix with backup"
    __hRow "oc logs" "Open /tmp/openclaw runtime log in VS Code"
    __hRow "le" "Gateway: 40-line log tail"
    __hRow "lo" "Gateway: 120-line full log"
    __hRow "lc" "Gateway: Clear all logs"
    __hRow "oc log-dir" "Change to OpenClaw logs folder"
    __hRow "oc sec" "Deep OpenClaw security audit"
    __hRow "oc docs" "Full-text search OpenClaw documentation"
    __hRow "oc cache-clear" "Remove /dev/shm telemetry caches"
    __hRow "oc diag" "5-point check: doctor, gateway, models, env, logs"
    __hRow "oc doctor-local" "Validate local gateway + llama.cpp"
    __hRow "oc failover" "Cloud LLM fallback (on|off|status)"
    __hRow "oc refresh-keys" "Re-import Windows API keys to WSL"

    __hSection "OPENCLAW — DATA & EXTENSIONS"
    __hRow "oc wk" "Change to OpenClaw workspace directory"
    __hRow "oc root" "Change to OpenClaw root config directory"
    __hRow "oc backup" "Snapshot workspace + agents to ZIP"
    __hRow "oc restore" "Restore from backup ZIP (--dry-run)"
    __hRow "oc cron" "Manage scheduled tasks (list|add|runs)"
    __hRow "oc skills" "Show installed/eligible skill modules"
    __hRow "oc plugins" "Manage plugins (list|doctor|enable|disable)"
    __hRow "oc usage" "Token and cost usage stats (default: 7d)"
    __hRow "oc channels" "Messaging channels (list|status|logs)"
    __hRow "oc browser" "Headless browser (status|start|stop)"
    __hRow "oc nodes" "Compute nodes (status|list|describe)"
    __hRow "oc sandbox" "Code sandboxes (list|recreate)"

    __hSection "OPENCLAW — LLM INTEGRATION"
    __hRow "oc local-llm" "Register local llama.cpp as provider"
    __hRow "oc sync-models" "Sync models.conf with provider scan"

    __hSection "OPENCLAW — TOOLS"
    __hRow "oc g" "Launch knowledge graph server in browser"
    ...
    __hRow "model default [N]" "Show/set default LLM model"
    ...
    __hRow "model recommend" "Rank scanned models for VRAM and use case"
    __hRow "model info N" "Display full details for model #N"
    __hRow "model bench" "Benchmark all on-disk models and compare TPS"
    __hRow "model bench-diff" "Compare two benchmark TSV runs"
    ...
    __hRow "model archive N" "Move model #N to archive and deregister"
    ...
    __hRow "commit: <msg>" "Git add, commit with message, and push"
    ...
    __hRow "cop-ask <msg>" "Non-interactive Copilot prompt"
    __hRow "cop-init" "Generate copilot-instructions.md for project"
