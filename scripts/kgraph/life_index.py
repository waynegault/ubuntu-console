...
def merge_relations(graph: Graph | dict, life_root: str | None = None) -> Graph:
    ...
    # Build slug-to-node-id mapping
    slug_to_node: dict[str, str] = {}
    for node in graph.nodes:
        if node.slug:
            slug_to_node[node.slug] = node.id
        if node.canonical_slug:
            slug_to_node[node.canonical_slug] = node.id
