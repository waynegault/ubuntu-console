...
IFS='|' read -r num name file size _qc _arch gpu_layers _ctx _thr _ba _ub _pa _fi _be _mm _fa _tps autotuned _isdef _vram <<< "$ENTRY"
...
bench_once() {
    ...
    local resp=$(curl -sS --max-time 180 http://127.0.0.1:8081/v1/chat/completions \
...
bench_ctx() {
    ...
    if [ "$samples" -eq 1 ]; then
        local tps=$(bench_once "$c" "$b" "$u") || { echo ""; return 1; }
    ...
    local t1=$(bench_once "$c" "$b" "$u") || t1=""
    t1=$(echo "$t1" | bc 2>/dev/null || echo "0")
    local t2=$(bench_once "$c" "$b" "$u") || t2=""
...
record_best() {
    ...
    local s=$(echo "$c * $tps" | bc 2>/dev/null || echo "0")
